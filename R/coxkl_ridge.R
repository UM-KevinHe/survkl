...
# data(ExampleData)
# result1 <- coxkl_ridge(z = ExampleData$z,
...
# result2 <- coxkl_ridge(z = ExampleData$z,
#                        delta = ExampleData$status,
#                        time = ExampleData$time,
#                        RS = rs,
#                        eta_list = seq(0, 5, 1),
...
#'
#' @export
coxkl_ridge <- function(z, delta, time, RS = NULL, beta = NULL, eta_list, tol=1.0e-7, Mstop = 50, 
                        lambda = NULL,
                        verbose = TRUE){
  ...
  if(is.null(lambda)) {
    lambda <- 1.0
    warning("No `lambda` provided; using default lambda = 1.0")
  } else if (length(lambda) != 1 || !is.finite(lambda) || lambda < 0)
    stop("`lambda` must be a single non-negative scalar.")
  ...
  likelihood_list <- numeric(length(eta_list))
  
  for (i in seq_along(eta_list)) {
    eta <- eta_list[i]
    
    beta_hat <- KL_Cox_Estimate_cpp_ridge(
      Z = z, delta = delta, theta_tilde = RS, eta = eta,
      tol = tol, maxit = Mstop, lambda = lambda
    )
    LP_hat <- as.numeric(z %*% beta_hat)
    
    beta_list[[i]]      <- beta_hat
    LP_list[[i]]        <- LP_hat
    likelihood_list[i]  <- pl_cal_theta(LP_hat, delta, time) 
  }
